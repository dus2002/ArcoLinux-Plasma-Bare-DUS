...
function AURInstalls() {
	sudo -u liveuser yay --noconfirm -S dolphin-root-git p7zip-gui p7zip-zstd-codec hfsprogs git-gui-menuentry cpupower-gui
}
...
function Kernel() {
	...
	cp /lib/modules/$kernelversion/vmlinuz /boot/vmlinuz-$linuxnumber-$arch
...
function getNewMirrorCleanAndUpgrade() {
    pacman-mirrors -a -m rank -t 5 -S unstable
    pacman -Sc --noconfirm
